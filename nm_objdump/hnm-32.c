...
void print_symbol_table32(Elf32_Shdr *section_header,
	...
	for (i = 0; i < symbol_count; i++)
	{
		    ...
		    ELF32_ST_TYPE(symbol.st_info) != STT_FILE)
		{
			...
			else if (symbol.st_shndx < SHN_LORESERVE)
			{
				...
				if (ELF32_ST_BIND(symbol.st_info) ==
				    STB_GNU_UNIQUE)
					symbol_type = 'u';
...
void process_elf_file32(char *file_path)
{
	...
	print_symbol_table32(&symbol_table_header,
			     symbol_table,
			     string_table,
			     section_headers);
